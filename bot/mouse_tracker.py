...
class MouseTracker:
    ...
    def _record(self, event_type: str, x: int, y: int, extra: Optional[Dict[str, Any]] = None) -> bool:
        """
    ...
    def click(self, x: int, y: int, selector: str = "") -> bool:
        """마우스 클릭 기록"""
        return self._record("click", x, y, extra={"selector": selector} if selector else None)

    def drag(self, start_x: int, start_y: int, end_x: int, end_y: int, duration_ms: int = 0) -> bool:
        """마우스 드래그 기록"""
